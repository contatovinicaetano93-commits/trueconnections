      ...
      <div className="grid gap-px overflow-hidden rounded-sm border border-line bg-line sm:grid-cols-2 lg:grid-cols-4">
        {portals.map((item, i) => (
          <Reveal key={item.title} delay={i * 0.04} y={24}>
            <a
              href={item.href}
              className="focus-ring group flex h-full min-h-[9.5rem] flex-col justify-between bg-smoke p-6 transition-colors duration-300 hover:bg-ash md:p-7"
              {...("external" in item && item.external
                ? { target: "_blank", rel: "noopener noreferrer" }
                : {})}
            >
              <span className="text-[0.65rem] tracking-[0.24em] text-gold/70 uppercase">
                {String(i + 1).padStart(2, "0")}
              </span>
              <div>
                <h2 className="display text-2xl text-parchment transition-colors group-hover:text-gold-soft">
                  {item.title}
                </h2>
                <p className="mt-2 text-sm text-mute">{item.subtitle}</p>
